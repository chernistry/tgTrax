"""
TgTrax entrypoint: CLI runner for tracking, analysis, and dashboard.

Modes:
- tracker: Runs the online-status tracker that writes activity to SQLite.
- report: Produces a console report of significant relationships and lags.
- dashboard: Starts the web dashboard (reads the same SQLite DB).
- auth: Interactive one-off login to create/validate a Telegram session.

Environment/config:
- .env provides Telegram API credentials and optional dashboard overrides.
- tgTrax_config.json provides app defaults (paths, toggles) with sane fallbacks.
"""
...
import json  # For config bootstrap and optional report export
import logging
...
# Basic logging setup (console). Advanced config can override later.
logging.basicConfig(level=getattr(logging, DEFAULT_LOG_LEVEL, logging.INFO))
logger = logging.getLogger("tgTrax.main")
...
def create_default_config_files_if_missing():
    ...
    if not os.path.exists(env_path):
        ...
        with open(env_path, "w") as f:
            ...
            f.write(
                "# Optional: Dashboard Configuration (Uncomment and set if needed)\\n"
            )
            f.write('# FLASK_HOST=\\"127.0.0.1\\"\\n')
            f.write('# FLASK_PORT=\\"5001\\"\\n\\n')
            f.write(
                "# Optional: Tracker Specific (Uncomment and set if needed for direct tracker.py usage)\\n"
            )
        ...
        print(
            f"INFO: Default .env file created at {env_path}. Please fill in your Telegram API credentials."
        )
    else:
    ...
    if not os.path.exists(config_json_path):
        print(
            f"INFO: tgTrax_config.json not found at {config_json_path}. Creating default config file..."
        )
        default_config_data = {
...
# Now that config files exist and .env is loaded, import runtime modules
...
LEGACY_SESSION = os.path.join(
    os.path.dirname(PROJECT_ROOT), f"{TG_SESSION_NAME}.session"
)
try:
    if os.path.exists(LEGACY_SESSION) and not os.path.exists(
        f"{SESSION_FILE_PATH}.session"
    ):
        import shutil
...
DASHBOARD_HOST = os.getenv(
    "FLASK_HOST", APP_CONFIG.get("dashboard_host", DEFAULT_DASHBOARD_HOST)
)
DASHBOARD_PORT = int(
    os.getenv(
        "FLASK_PORT", str(APP_CONFIG.get("dashboard_port", DEFAULT_DASHBOARD_PORT))
    )
)
DASHBOARD_DEBUG_STR = os.getenv(
    "FLASK_DEBUG", str(APP_CONFIG.get("dashboard_debug", DEFAULT_DASHBOARD_DEBUG))
)
...
REDIS_CACHE_TTL_SECONDS = int(
    APP_CONFIG.get("redis_cache_ttl_seconds", DEFAULT_REDIS_CACHE_TTL_SECONDS)
)

# --- End Application Configuration Loading ---


async def run_tracker_mode(args: argparse.Namespace) -> None:
    """
    Run the user-activity tracker using target usernames from `TARGET_USERS`.

    Notes:
    - CorrelationTracker manages its own Telethon client lifecycle.
    - This function validates input and delegates to the tracker.
    """
    logger.info("Starting user tracker mode...")
    target_users_env = os.getenv("TARGET_USERS")
    if not target_users_env:
        logger.error(
            "TARGET_USERS environment variable not set. CorrelationTracker mode cannot start."
        )
        return
    target_usernames_list = [
        u.strip() for u in target_users_env.split(",") if u.strip()
    ]
    if not target_usernames_list:
        logger.error(
            "No valid target usernames parsed from TARGET_USERS for CorrelationTracker."
        )
        return
...
def run_dashboard_mode_entry(args: argparse.Namespace) -> None:
    """Start the Flask dashboard, serving metrics from the SQLite database."""
    logger.info(
        "Starting dashboard mode on %s:%s (Debug: %s)",
        DASHBOARD_HOST,
        DASHBOARD_PORT,
        DASHBOARD_DEBUG,
    )
...
def handle_report_mode(args: argparse.Namespace) -> None:
    """
    Generate a console report for significant pairs with best lag and metrics.

    Steps:
    - Load users from `TARGET_USERS` or discover distinct users from the DB.
    - Build a TemporalAnalyzer over resampled activity for correlation/lag scan.
    - Render a compact table; optionally export as JSON if `--export` is set.
    """
    ...
    if target_users_env:
        ...
        try:
            ...
            logger.error("Failed to discover users from DB: %s", e)
            user_list = []

    if not user_list:
        logger.error("No users to analyze. Set TARGET_USERS or populate DB.")
        return

    tracker = UserTracker(target_usernames=user_list, db_path=DB_PATH)
    df = tracker.get_activity_data()
    if df.empty:
        logger.warning("No activity data found. Abort report.")
        return
    ...
    for idx, (pair, meta) in enumerate(pairs[: args.top], start=1):
        ...
        jv = (
            ...
            if jacc.empty
                ...
                float(jacc.at[u, v])
                if (u in jacc.index and v in jacc.columns)
                else None
            )
        )
        cv = (
            ...
            if corr.empty
                ...
                float(corr.at[u, v])
                if (u in corr.index and v in corr.columns)
                else None
            )
        )
        table.add_row(
...
# (Deprecated 'scrape' and 'unreact' modes removed.)
...
async def main_async_runner() -> None:
    ...
    Design:
    - Asynchronous to support modes that require a Telegram client.
    - Tracker manages its own client; other async modes use `get_client` here.
    - Synchronous dashboard mode is invoked directly.
    """
    print("DEBUG: main_async_runner started")
    parser = argparse.ArgumentParser(
        description="TgTrax - Telegram User Activity Tracker and Analyzer."
    )
    subparsers = parser.add_subparsers(
        dest="mode", help="Available modes of operation", required=True
    )

    # (Removed deprecated scrape/unreact defaults)

    # Tracker mode
    tracker_parser = subparsers.add_parser(
        "tracker", help="Run user activity tracking service."
    )
    tracker_parser.set_defaults(func=run_tracker_mode)

    # Dashboard mode
    dashboard_parser = subparsers.add_parser(
        "dashboard", help="Run the web interface dashboard."
    )
    dashboard_parser.set_defaults(func=run_dashboard_mode_entry)

    # Report mode (CLI analysis)
    report_parser = subparsers.add_parser(
        "report", help="Generate TUI analysis report with significance and lags."
    )
    report_parser.add_argument(
        "--period", type=str, default="1min", help="Resample period, e.g. 1min, 5T"
    )
    report_parser.add_argument(
        "--method",
        type=str,
    ...
    report_parser.add_argument(
        "--ewma-alpha", type=float, default=None, help="EWMA smoothing alpha (0,1]"
    )
    report_parser.add_argument(
        "--seasonal-adjust",
        action="store_true",
        help="Enable STL seasonal/trend removal",
    )
    report_parser.add_argument(
        "--max-lag-minutes",
        type=int,
        default=15,
        help="Max lag (minutes) for cross-correlation",
    )
    report_parser.add_argument(
        "--fdr-alpha", type=float, default=0.05, help="FDR alpha for q-values"
    )
    report_parser.add_argument(
        "--q-threshold",
        type=float,
        default=0.05,
        help="q-value threshold to accept pair",
    )
    report_parser.add_argument(
        "--min-abs-corr",
        type=float,
        default=0.0,
        help="Minimum absolute cross-corr at best lag",
    )
    report_parser.add_argument(
        "--corr-threshold",
        type=float,
        default=0.3,
        help="Threshold for plain correlation",
    )
    report_parser.add_argument(
        "--jaccard-threshold", type=float, default=0.18, help="Threshold for Jaccard"
    )
    report_parser.add_argument(
        "--top", type=int, default=50, help="Top N pairs to display"
    )
    report_parser.add_argument(
        "--export", type=str, default=None, help="Export report JSON to path"
    )
    ...
    auth_parser = subparsers.add_parser(
        "auth", help="Interactive console login to Telegram"
    )
    auth_parser.add_argument("--phone", type=str, help="Phone number with country code")
    auth_parser.add_argument(
        "--code", type=str, help="Login code received from Telegram"
    )
    auth_parser.add_argument("--password", type=str, help="2FA password if enabled")
    auth_parser.add_argument(
        "--session-name", type=str, help="Override session file name"
    )
    ...
    try:
        if asyncio.iscoroutinefunction(selected_func):
            ...
            client_instance = await get_client(
                SESSION_FILE_PATH, TG_API_ID, TG_API_HASH, TG_PHONE_NUMBER
            )
            if not client_instance:
                logger.error("Failed to initialize Telegram client for async mode.")
                return
        ...
        logger.error(
            "Critical error during operation in mode '%s': %s",
            getattr(args, "mode", "?"),
            e,
        )
    finally:
        if client_instance and asyncio.iscoroutinefunction(selected_func):
            logger.info("Disconnecting Telegram client from main_async_runner...")
            await client_instance.disconnect()
            logger.info("Client disconnected.")


async def handle_auth_console(args: argparse.Namespace) -> None:
    """
    Minimal interactive console-based authentication to create a session file.

    Prompts for phone, code, and (if needed) 2FA password, then saves the
    session under the configured `sessions` directory for later reuse.
    """
    ...
    if await client.is_user_authorized():
        logger.info(
            "Telegram session authorized. You can now run the tracker/API."
        )
    else:
        logger.error("Authorization failed. Please verify the code/password and retry.")
...
if __name__ == "__main__":
    try:
        ...
        logger.warning("Process interrupted by user (KeyboardInterrupt). Exiting...")
        sys.exit(0)
    except Exception as e:
        logger.error("Unhandled critical exception in __main__: %s", e)
        sys.exit(1)
