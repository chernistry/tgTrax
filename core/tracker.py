...
class CorrelationTracker:
    ...
    async def _handle_user_update(self, event: events.UserUpdate) -> None:
        ...
        if username:
            ...
            # This is common for updates about non-tracked users
            tui.tui_print_debug(
                f"Received update for user ID {user_id}, not in target map. Event: {event!r}"
            )
    ...
    async def _poll_user_statuses(self) -> None:
        ...
        while not self.shutdown_event.is_set() and self.is_running:
            ...
            if not self.client.is_connected():
                ...
                try:
                    await asyncio.wait_for(
                        ...
                        timeout=float(USER_STATUS_POLL_INTERVAL_SECONDS)
                    )
                except asyncio.TimeoutError:
                    pass # Normal timeout, continue to next iteration attempt
                except asyncio.CancelledError:
            ...
            try:
                await asyncio.wait_for(self.shutdown_event.wait(), timeout=float(USER_STATUS_POLL_INTERVAL_SECONDS))
                if self.shutdown_event.is_set(): break # Shutdown triggered during wait
            except asyncio.TimeoutError:
    ...
    async def start_tracking(self) -> None:
        ...
        while self.is_running and not self.shutdown_event.is_set():
            try:
                ...
                loop_counter = 0
                while self.client.is_connected() and self.is_running and not self.shutdown_event.is_set():
                    try:
                ...
                if not self.client.is_connected() and self.is_running and not self.shutdown_event.is_set():
    ...
    async def stop_tracking(self) -> None:
        ...
        if self.client:
            ...
            try: self.client.remove_event_handler(self._handle_user_update, events.UserUpdate)
...
async def main_tracker_entrypoint(usernames_env_var_value: str) -> None:
    ...
    try:
        ...
        if tracker and (tracker.is_running or tracker.shutdown_event.is_set() is False):
